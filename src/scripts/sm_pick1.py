	...
	def execute(self, userdata):
		...
		while not rospy.is_shutdown():
			...
			# finished with aborted
				...
				sss.stop("base")
				s112=sss.move("base",[-3.24,1.16,2.8])
				if (s112.get_state() != 3): 
					s111=sss.move("base", [0, 6.4, 2.0])
					if (s111.get_state() != 3):
						sss.move("base",[1.05,1.38,1.4])
					else:
						return 'not_reached'
				else:
					return 'not_reached'
...
class AThome(smach.State):
	...
	def execute(self,userdata):
		...
		sss.move("torso","nod")
		sss.move("head", "back")
		s10=sss.move("tray", "deliverdown",False)
		...
		while not rospy.is_shutdown():
			...
			# finished with aborted
				...
				return 'not_reached'
			# finished with preempted or canceled
			elif (s10.get_state() == 2) or (s10.get_state() == 8):
				#sss.stop("arm")
				sss.stop("tray")
				return 'not_reached'
			# return with error
			elif (s10.get_error_code() > 0):
				print "error_code = " + str(s8.get_error_code())
				sss.set_light("light", 'red')
				return 'failed'
